...
class FactoryUR5eEnv(DirectRLEnv):
    ...
    def get_handheld_asset_relative_pose(self):
        ...
        if self.cfg_task.name == "peg_insert":
            ...
            held_asset_relative_pos[:, 2] -= self.cfg_task.robot_cfg.ur5e_fingerpad_length
        elif self.cfg_task.name == "gear_mesh":
            held_asset_relative_pos = torch.zeros_like(self.held_base_pos_local)
            gear_base_offset = self._get_target_gear_base_offset()
    ...
    def randomize_initial_state(self, env_ids):
        ...
        if self.cfg_task.name == "gear_mesh" and type(self.cfg_task) == GearMesh and self.cfg_task.add_flanking_gears:
            ...
            self._small_gear_asset.write_root_link_pose_to_sim(small_gear_state[:, 0:6], env_ids=env_ids)
            self._small_gear_asset.write_root_com_velocity_to_sim(small_gear_state[:, 6:], env_ids=env_ids)
            self._small_gear_asset.reset()

            large_gear_state = self._large_gear_asset.data.default_root_state.clone()[env_ids]
            large_gear_state[:, 0:6] = fixed_state[:, 0:6]
            large_gear_state[:, 6:] = 0.0  # vel
            self._large_gear_asset.write_root_link_pose_to_sim(large_gear_state[:, 0:6], env_ids=env_ids)
            self._large_gear_asset.write_root_com_velocity_to_sim(large_gear_state[:, 6:], env_ids=env_ids)
