...
def compute_dof_torque(
    ...
    distance_to_default_dof_pos = default_dof_pos_tensor - dof_pos[:, :6]
    distance_to_default_dof_pos = (distance_to_default_dof_pos + math.pi) % (
        2 * math.pi
    ) - math.pi  # normalize to [-pi, pi]
    u_null = cfg.ctrl.kd_null * -dof_vel[:, :6] + cfg.ctrl.kp_null * distance_to_default_dof_pos
    u_null = arm_mass_matrix @ u_null.unsqueeze(-1)
    torque_null = (torch.eye(6, device=device).unsqueeze(0) - torch.transpose(jacobian, 1, 2) @ j_eef_inv) @ u_null
    dof_torque[:, 0:6] += torque_null.squeeze(-1)
...
def get_pose_error(
    ...
    """Compute task-space error between target UR5e fingertip pose and current pose."""
